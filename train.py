...
if args.alg == 'FedAvg' or args.alg == 'FedProx' or args.alg == 'Moon':  
    ...
    if args.alg == 'DPMS':
        ...
        for epoch in tqdm(range(args.pretrain)):
            ...
            for idx in idxs_users:
                ...
                w, loss = local_model.update_weights(
                    model=copy.deepcopy(global_model), global_round=epoch, u=0.05, device=device)
        ...
        for epoch in tqdm(range(args.num_epochs)):
            ...
            for idx in idxs_users:
                ...
                if args.alg == 'FedVAE':
                    w, loss = local_model.update_weights(model=copy.deepcopy(global_model), global_round=epoch, u=0.05, device=device)
            ...
            for cnt, (X,y) in enumerate(loader_test):
                X = X.to(device)
                y = y.double().to(device)
                if args.alg == 'FedAvg' or args.alg == 'Moon' or args.alg == 'FedProx':
                    p = global_model(X)
                else:
                    m = torch.nn.Sigmoid()
                    X = m(X)
